...
fn main() {
    ...
    #[rustfmt::skip]
    let vertices: [f32; 16] = [
        -1.0,  1.0, -1.0,  1.0, // 0: top-left pos, uv
        -1.0, -1.0, -1.0, -1.0, // 1: bottom-left pos, uv
         1.0, -1.0,  1.0, -1.0, // 2: bottom-right pos, uv
         1.0,  1.0,  1.0,  1.0, // 3: top-right pos, uv
    ];

    // 2 floats for position (x, y) then two floats for uv (s, t)
    let vertex_layout = vec![2, 2];
